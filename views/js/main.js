...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
	  var allPizzaContainer = document.getElementsByClassName("randomPizzaContainer");		//changed to getElementsByClassName and removed from for loop
	  var allPizzaLength = allPizzaContainer.length;		//removed .length from for loop
	  var dx = determineDx(allPizzaContainer[0], size);		//removed dx from for loop
      var newwidth = (allPizzaContainer[0].offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');		//changed to getElementsByClassName
  var itemsLength = items.length;		//created itemsLength and removed items.length from for loop
  ...
  for (var i = 0; i < itemsLength; i++) {			//inserted itemsLength
	...
	items[i].style.WebkitTransform = 'translateX(' + (items[i].basicLeft + 100) * phase + 'px)'; //removed style.left & basicLeft. used transform & translateX
	items[i].style.MozTransform = 'translateX(' + (items[i].basicLeft + 100) * phase + 'px)';
	items[i].style.OTransform = 'translateX(' + (items[i].basicLeft + 100) * phase + 'px)';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 30; i++) {			//reduced to 30
